<template>
  <Teleport to="body">
    <div v-if="open" class="ob-overlay" :class="{ 'ob-overlay--dim': isGlobalDim }" @click.self="handleSkip" role="dialog" aria-modal="true">
      <!-- Spotlight highlight -->
...
const activeStep = computed(() => props.steps[Math.min(stepIndex.value, Math.max(0, props.steps.length - 1))])
...
function calcRect() {
  ...
  if (!id) { currentRect.value = null; positionCardCenter(); return }
  const el = document.getElementById(id)
  if (!el) { currentRect.value = null; positionCardCenter(); return }
...
function next() {
  if (isLast.value) { closeTour(true); return }
  stepIndex.value += 1
  nextTick(calcRect)
}
...
.ob-overlay--dim { background: rgba(0,0,0,0.45); }

.ob-spotlight {
  position: absolute;
  border-radius: 15px;
  box-shadow: 0 0 0 9999px rgba(0,0,0,0.45), 0 0 0 2px #409eff inset;
  pointer-events: none;
...
.ob-card__title { margin: 0 0 6px 0; color: var(--color-text-primary); font-size: 16px; font-weight: 700; }
.ob-card__desc { margin: 0; color: var(--color-text-secondary); font-size: 14px; line-height: 1.6; }
...
/* Step transition */
.ob-step-enter-from { opacity: 0; transform: translateY(-6px); }
.ob-step-enter-active { transition: all 160ms ease; }
.ob-step-enter-to { opacity: 1; transform: translateY(0); }
.ob-step-leave-from { opacity: 1; transform: translateY(0); }
.ob-step-leave-active { transition: all 140ms ease; }
.ob-step-leave-to { opacity: 0; transform: translateY(6px); }
</style>
